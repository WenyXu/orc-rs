...
impl<R: Read + Seek> Iterator for ArrowReader<R> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.current_stripe {
            Some(mut stripe) => match stripe.next() {
                Some(batch) => Some(batch.map_err(|err| ArrowError::ExternalError(Box::new(err)))),
                None => None,
            },
            None => match self
                .cursor
...
impl NaiveStripeDecoder {
    fn decode_next_batch(&mut self) -> Result<Option<RecordBatch>> {
        ...
        for decoder in self.decoders.as_mut() {
            match decoder {

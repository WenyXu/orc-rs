...
impl TypeDescriptionInner {
    ...
    pub fn get_field(&self, name: &str) -> Option<Arc<TypeDescription>> {
        ...
        idx.map_or(None, |idx| {
            self.children.as_ref().unwrap().get(idx).cloned()
        })
    }
}
...
pub fn create_schema(types: &[Type], root_column: usize) -> Result<Arc<TypeDescription>> {
    ...
    match root.kind() {
        Kind::Struct => {
            let mut td = Arc::new(TypeDescription::new(STRUCT.clone(), root_column));
            let sub_types = &root.subtypes;
            let fields = &root.field_names;
            for (idx, column) in sub_types.into_iter().enumerate() {
                let mut child = create_schema(types, *column as usize)?;
